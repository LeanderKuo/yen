...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: getMetadataAlternates(
      `/gallery/items/${categorySlug}/${slug}`,
      locale,
    ),
    openGraph: {
      title,
...
export default async function GalleryItemPage({ params }: PageProps) {
  ...
  if (!item) {
    // If the item slug is unique across categories, use permanent redirect (308) to its canonical category path.
    // ARCHITECTURE.md §3.11: canonicalization must use permanentRedirect(), not redirect() (307).
