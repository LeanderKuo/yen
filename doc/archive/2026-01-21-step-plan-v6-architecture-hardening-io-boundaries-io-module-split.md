# 2026-01-21 - Step Plan V6（Architecture Hardening：IO boundaries + IO module split）(Archive)
...
- What shipped: PR-22..PR-25（移除 app layer 直接 DB query；拆分 oversized IO modules；清掉 lint warning）
- Why archive: `doc/meta/STEP_PLAN.md` 只保留 active drift / 下一步；已完成內容移到 `doc/archive/*` 避免干擾
- Repo verification (2026-01-21): `npm test`（1083 pass）, `npm run lint`, `npm run type-check` 全通過
